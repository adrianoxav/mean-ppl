...
export class CursoEditComponent implements OnInit {
 ...
 this.http.get('http://localhost:443/materia/'+this.materia,httpOptions).subscribe(data => {
   console.log(data);
datos=data;
   ...
   onchange1(newValue){
    ...
    /*    this.http.get('http://localhost:443/materia',httpOptions).subscribe(data => {
          console.log(data);
          this.materias = data;
        });*/
        console.log(this.curso);
      ...
      getCurso(id) {
        ...
        this.http.get('http://localhost:443/curso/'+id,httpOptions).subscribe(data => {
          ...
          for (let i of this.curso.profesores){
            let ver:any;
          this.http.get('http://localhost:443/user/'+i,httpOptions).subscribe(data => {
               console.log(data);
               ver=data;
               if(ver.email=="admin"){}
               else{
             ...
             this.http.get('http://localhost:443/estudiantes/'+i,httpOptions).subscribe(data => {
                  console.log(data);
    ...
    //    this.curso.updated_date = Date.now();
        this.http.put('http://localhost:443/curso/'+id, this.curso,httpOptions)
          .subscribe(res => {
              let id = res['_id'];
      ...
      saveAssignation(asig) {
        ...
        this.http.post('http://localhost:443/asignacion', asig,httpOptions)
          .subscribe(res => {
              let id = res['_id'];
...
let assign:any;
let r:any;
      this.http.post('http://localhost:443/api/register', l, httpOptions)
        .subscribe(res => {
            //console.log(res.msg);
            r=res;
            let idUser = res['_id'];
let assignation:any;
            if(r.msg=="email already exists."){
              let dat:any;
              this.http.get('http://localhost:443/user/email/'+l.email, httpOptions).subscribe(data => {
                console.log(data);
                dat=data;
                 assignation = {
                     'idCurso': this.id,
                   ...
                   this.http.post('http://localhost:443/asignacion', assignation,httpOptions)
                     .subscribe(res => {
                         console.log(res);
                       }, (err) => {
                         console.log(err);
