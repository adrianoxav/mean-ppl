...
datos=data;
   ...
   onchange1(newValue){
    ...
    /*    this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/materia',httpOptions).subscribe(data => {
      ...
      getCurso(id) {
        let curs:any;
        this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/curso/'+id,httpOptions).subscribe(data => {
          curs=data;
          this.curso = curs;
          console.log(this.curso);
          for (let i of this.curso.profesores){
            let ver:any;
          this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/user/'+i,httpOptions).subscribe(data => {
               console.log(data);
               ver=data;
               if(ver.email=="admin"){}
               else{
    ...
    //    this.curso.updated_date = Date.now();
        this.http.put('http://www.aprendizajeactivo.espol.edu.ec:3000/curso/'+id, this.curso,httpOptions)
          .subscribe(res => {
              let id = res['_id'];
      ...
      saveAssignation(asig) {
        ...
        this.http.post('http://www.aprendizajeactivo.espol.edu.ec:3000/asignacion', asig,httpOptions)
          .subscribe(res => {
              let id = res['_id'];
...
let assign:any;
let r:any;
      this.http.post('http://www.aprendizajeactivo.espol.edu.ec:3000/api/register', l, httpOptions)
        .subscribe(res => {
            //console.log(res.msg);
            r=res;
            let idUser = res['_id'];
let assignation:any;
            if(r.msg=="email already exists."){
              let dat:any;
              this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/user/email/'+l.email, httpOptions).subscribe(data => {
                console.log(data);
                dat=data;
                 assignation = {
                     'idCurso': this.id,
                   ...
                   this.http.post('http://www.aprendizajeactivo.espol.edu.ec:3000/asignacion', assignation,httpOptions)
                     .subscribe(res => {
                         console.log(res);
                       }, (err) => {
                         console.log(err);
