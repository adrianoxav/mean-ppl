...
export class PreguntaAssessmentEditComponent implements OnInit {
  ...
  getPregunta_assessment(id) {
    this.http.get('http://www.aprendizajeactivo.espol.edu.ec:80/pregunta_assessment/'+id, httpOptions).subscribe(data => {
  ...
  //    this.pregunta_assessment.updated_date = Date.now();
    this.http.put('http://www.aprendizajeactivo.espol.edu.ec:80/pregunta_assessment/'+id, this.pregunta_assessment, httpOptions)
      .subscribe(res => {
          let id = res['_id'];
