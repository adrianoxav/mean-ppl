...
export class CuestionarioEditComponent implements OnInit {
    ...
    getCuestionario(id) {
      this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/cuestionario/'+id).subscribe(data => {
    ...
    updateCuestionario(id) {
      ...
      this.http.put('http://www.aprendizajeactivo.espol.edu.ec:3000/cuestionario/'+id, this.cuestionario,httpOptions)
        .subscribe(res => {
            let id = res['_id'];
