...
export class MateriaEditComponent implements OnInit {
  ...
  lista: any;
      ...
      getCurso(id) {
        this.http.get('http://www.ppl.espol.edu.ec:443/materia/'+id,httpOptions).subscribe(data => {
          this.materia = data;
        });
      }
    ...
    //    this.curso.updated_date = Date.now();
        this.http.put('http://www.ppl.espol.edu.ec:443/materia/'+this.id, this.materia,httpOptions)
        .subscribe(res => {
          console.log(res);
            let id = res['_id'];
