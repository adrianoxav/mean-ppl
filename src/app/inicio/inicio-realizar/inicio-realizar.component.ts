...
constructor(private router: Router, private route: ActivatedRoute, private http: HttpClient) { }
  ...
  getAssessmentDetail(id) {
    this.http.get('http://www.ppl.espol.edu.ec:443/evaluacion_estudiantepeer/'+id, httpOptions).subscribe(data => {
    ...
    {  this.router.navigate(['/inicio']);}
      ...
      console.log(data);
      this.http.get('http://www.ppl.espol.edu.ec:443/cuestionario/'+this.assessment.idEvaluacion.idCuestionario, httpOptions).subscribe(data => {
        this.assessment.idEvaluacion.idCuestionario = data;
        console.log(data);
        for(let preg of this.assessment.idEvaluacion.idCuestionario.preguntas){
          let pregunta:any;
          let dat:any;
          this.http.get('http://www.ppl.espol.edu.ec:443/pregunta/'+preg, httpOptions).subscribe(data => {
            console.log(data);
            dat=data;
...
/*     index = this.cursosSeleccionados.findIndex(num => num == id);
 ...
 /*     index = this.cursosSeleccionados.findIndex(num => num == id);
    ...
    saveassessment(){
      ...
      this.http.put('http://www.ppl.espol.edu.ec:443/evaluacion_estudiantepeer/peer/'+this.assessment._id, this.assessment, httpOptions)
        .toPromise().then(res => {
            console.log(res);
