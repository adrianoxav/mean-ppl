...
constructor(private router: Router, private route: ActivatedRoute, private http: HttpClient) { }
  ...
  getAssessmentDetail(id) {
    this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/evaluacion_estudiantepeer/'+id, httpOptions).subscribe(data => {
      this.assessment = data;
      console.log(data);
      this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/cuestionario/'+this.assessment.idEvaluacion.idCuestionario, httpOptions).subscribe(data => {
        this.assessment.idEvaluacion.idCuestionario = data;
        console.log(data);
        for(let preg of this.assessment.idEvaluacion.idCuestionario.preguntas){
          let pregunta:any;
          let dat:any;
          this.http.get('http://www.aprendizajeactivo.espol.edu.ec:3000/pregunta/'+preg, httpOptions).subscribe(data => {
            console.log(data);
            dat=data;
...
/*     index = this.cursosSeleccionados.findIndex(num => num == id);
 ...
 /*     index = this.cursosSeleccionados.findIndex(num => num == id);
    ...
    saveassessment(){
      ...
      this.http.put('http://www.aprendizajeactivo.espol.edu.ec:3000/evaluacion_estudiantepeer/peer/'+this.assessment._id, this.assessment, httpOptions)
        .subscribe(res => {
            //let id = res['_id'];
          }, (err) => {
            console.log(err);
