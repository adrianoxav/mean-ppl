...
export class AssessmentEditComponent implements OnInit {
  ...
  getAssessment(id) {
    this.http.get('http://aprendizajeactivo.espol.edu.ec:443/assessment/'+id).subscribe(data => {
...
//    this.assessment.updated_date = Date.now();
    this.http.put('http://aprendizajeactivo.espol.edu.ec:443/assessment/'+id, this.assessment)
      .subscribe(res => {
          let id = res['_id'];
