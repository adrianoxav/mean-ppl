...
export class LoginComponent implements OnInit {
  ...
  login() {
  this.http.post('http://www.aprendizajeactivo.espol.edu.ec:80/api/signinEstudiante',this.loginData).subscribe(resp => {
    this.data = resp;
    console.log(resp);
    ...
    console.log(err);
    this.http.post('http://www.aprendizajeactivo.espol.edu.ec:80/api/signin',this.loginData).subscribe(resp => {
      this.data = resp;
      console.log(resp);
      localStorage.setItem('jwtToken', this.data.token);
      localStorage.setItem('nombres', this.data.user.nombres);
