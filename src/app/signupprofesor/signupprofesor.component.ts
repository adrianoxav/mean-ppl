...
this.http.post('http://localhost:443/api/signup',this.signupData).subscribe(resp => {
  console.log(resp);
  res=resp;
  let idUser = resp['_id'];
  ...
  if(res.msg=="email already exists."){
    ...
    this.http.get('http://localhost:443/users/email/'+this.signupData.email).subscribe(data => {
