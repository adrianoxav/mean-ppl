...
export class EvaluacionEstudianteEditComponent implements OnInit {
          ...
          getEvaluacion_estudiante(id) {
            this.http.get('http://www.ppl.espol.edu.ec:443/evaluacion_estudiante/'+id,httpOptions).subscribe(data => {
        ...
        //    this.evaluacion.updated_date = Date.now();
            this.http.put('http://www.ppl.espol.edu.ec:443/evaluacion_estudiante/'+id, this.evaluacion_estudiante,httpOptions)
              .subscribe(res => {
                  let id = res['_id'];
